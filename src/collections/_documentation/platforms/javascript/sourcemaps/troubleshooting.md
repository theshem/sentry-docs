...
For uploaded source maps to be located and applied, the release needs to be created by the CLI or API (and the correct artifacts uploaded with it), and your SDK configuration needs to specify the name of that newly-created release.

To verify this, open up the issue from the Sentry UI and check if you've configured the release. If it says "_not configured_" or "_N/A_" next to **Release** on the right-hand side of the screen (or if you do not see a `release` tag in the list of tags), you'll need to go back and [tag your errors]({%- link _documentation/workflow/releases.md -%}#tag-errors). If this is properly set up, you'll see "Release: my_example_release". 

### Verify artifacts are uploaded

Once your release is properly configured, and issues are tagged, from within an issue you can then click on the release >> Artifacts (or **Releases >> your specific release >> Artifacts**) to check that you've uploaded your source maps and the associated files to the correct release.
...
If you upload artifacts **after** Sentry captures an error, Sentry will not go back and retroactively apply any source annotations to those errors. Only new errors triggered after you uploaded the artifact will be affected.
...
For an individual artifact, Sentry accepts a max file size of **40 MB**.
...
The Sentry API currently only works with source maps and source files that you've uploaded as plain text (UTF-8 encoded). If you upload the files in a compressed format (e.g. gzip), Sentry will not interpret them correctly.

This sometimes occurs with build scripts and plugins that produce pre-compressed minified files. For example, Webpack’s [compression plugin](https://github.com/webpack/compression-webpack-plugin). You’ll need to disable such plugins and perform the compression _after_ you've uploaded the generated source maps/source files to Sentry.
